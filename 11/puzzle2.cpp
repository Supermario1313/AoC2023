...
int main() {
    ...
    for (std::size_t k = 0; k < galaxies.size(); k++) {
        for (std::size_t l = k + 1; l < galaxies.size(); l++) {
            ...
            distances_sum += (max_i - min_i) + (max_j - min_j) + 999999uz * (empty_lines_until[max_i] - empty_lines_until[min_i]) + 999999uz * (empty_columns_until[max_j] - empty_columns_until[min_j]);
        }
    }
